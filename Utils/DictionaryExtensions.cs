...
public static class DictionaryExtensions
{
  ...
  public static Dictionary<T2, List<T1>> GroupToDictionary<T1, T2>(this IEnumerable<T1> self, Func<T1, T2> key)
  {
    return self.GroupBy(it => key(it), it => it).ToDictionary(it => it.Key, it => it.ToList());
  }
